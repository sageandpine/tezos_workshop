...
type deposit_type is michelson_pair(michelson_pair(address, "from", nat, "WXTZ"), "", contract(mutez), "")
type redeem_type is michelson_pair(michelson_pair(address, "to", mutez, "XTZ"), "", contract(wxtz), "")
...
///deposit mutez  into storage then convert/"mint" wxtz_token (wxtz_token issued == mutez_deposit)
function deposit(const owner : address; const receiver : address; const value : nat; const s : storage) : storage is
block {
    const src : account = getAccount(owner, s);
    const dst : account = getAccount(receiver, s);
    if Tezos.sender = owner then skip else block {
...
| Deposit(v) -> ((nil : list(operation)), deposit( v.0, v.1.0, v.1.1, s))
| Redeem(v) -> ((nil : list(operation)), redeem( v.0, v.1.0, v.1.1, s))
